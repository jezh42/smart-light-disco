...
const whiteButton = new Gpio(26, 'in', 'both');
const yellowButton = new Gpio(5, 'in', 'both');
const blueButton = new Gpio(10, 'in', 'both');
const greenButton = new Gpio(6, 'in', 'both');
...
(async () => {
   ...
   // Blue Button => blurple rgb(255,100,42)
   blueButton.watch(async (err, value) => {
      if (err) {
         throw err;
      }

      buttonPress("blue", value);

      if (value ^ 1) {
         console.log("Start blurple");
         await smartBulbDevice.lighting.setLightState({
            transition_period: 0,
            hue: 353,
            saturation: 92,
   ...
   greenButton.watch(async (err, value) => {
      if (err) {
         throw err;
      }
